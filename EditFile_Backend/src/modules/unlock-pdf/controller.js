...
export const unlockPdf = asyncHandler(async (req, res) => {
  ...
  logger.info(`Unlock PDF request: ${jobId}`);
  ...
  try {
    ...
    logger.info(`Unlock PDF job created: ${job.id} in ${duration}ms`);
    ...
    logger.error(`Unlock PDF failed for job ${jobId}:`, error);
    throw error;
  }
});
