...
const ensureOutputExtension = (fileName: string, format: PdfToWordOutputFormat) =>
...
export default function PDFToWord() {
  ...
  const [outputFormat, setOutputFormat] = useState<PdfToWordOutputFormat>('docx');
  const [isConverting, setIsConverting] = useState(false);
  const [requestError, setRequestError] = useState<string | null>(null);
  const [requestSuccess, setRequestSuccess] = useState<string | null>(null);
  const filesRef = useRef<PdfToWordFile[]>([]);
  ...
  const handleFiles = useCallback((incomingFiles: File[]) => {
    ...
    const createdFiles: PdfToWordFile[] = validFiles.map((file) => ({
      id: createFileId(),
      file,
      name: file.name,
      size: file.size,
  ...
  const handleSingleDownload = (file: PdfToWordFile) => {
    if (!file.downloadUrl) {
      return;
    }
  ...
  const autoDownloadZip = async (completedFiles: PdfToWordFile[]) => {
    const zip = new JSZip();
    ...
    triggerBlobDownload(zipBlob, `pdf-to-word-${timestamp}.zip`);
  };
  ...
  const handleConvert = async () => {
    ...
    const updateFileState = (
      fileId: string,
      updater: (target: PdfToWordFile) => PdfToWordFile
    ) => {
      setFiles((prev) => {
    ...
    try {
      if (latestFiles.length > 1 && completedFiles.length > 1) {
        ...
        setRequestSuccess(
          'Conversion completed. ZIP download started automatically.'
        );
            ...
            >
              <div className="sticker-card p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    { value: 'docx', label: 'DOCX (Recommended)' },
                    { value: 'doc', label: 'DOC (Legacy)' },
                    { value: 'rtf', label: 'RTF' },
                  ].map((format) => (
                    <button
                      key={format.value}
                      onClick={() => setOutputFormat(format.value as PdfToWordOutputFormat)}
