...
const ensureJpgOutputName = (fileName: string) => {
...
const resolvePdfToJpgOutputName = (
  backendFileName: string | null | undefined,
  fallbackPdfName: string
) => {
  const normalizedBackendName = String(backendFileName || '').trim();
  if (normalizedBackendName) {
    return normalizedBackendName;
  }

  return ensureJpgOutputName(fallbackPdfName);
};
...
export default function PDFToJPG() {
  const [files, setFiles] = useState<PdfToJpgFile[]>([]);
  ...
  const filesRef = useRef<PdfToJpgFile[]>([]);
  ...
  const handleFiles = useCallback((incomingFiles: File[]) => {
    ...
    const createdFiles: PdfToJpgFile[] = validFiles.map((file) => ({
      id: createFileId(),
      file,
      name: file.name,
      size: file.size,
  ...
  const handleSingleDownload = (file: PdfToJpgFile) => {
    if (!file.downloadUrl) {
      return;
    }

    const outputName = file.outputName || ensureJpgOutputName(file.name);
    startFileDownload(file.downloadUrl, outputName);
  };

  const autoDownloadZip = async (completedFiles: PdfToJpgFile[]) => {
    const zip = new JSZip();
    ...
    await Promise.all(
      completedFiles.map(async (file) => {
        ...
        zip.file(file.outputName || ensureJpgOutputName(file.name), outputBlob);
      })
    );

    const zipBlob = await zip.generateAsync({ type: 'blob' });
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    triggerBlobDownload(zipBlob, `pdf-to-jpg-${timestamp}.zip`);
  };
  ...
  const handleConvert = async () => {
    ...
    const updateFileState = (
      fileId: string,
      updater: (target: PdfToJpgFile) => PdfToJpgFile
    ) => {
      setFiles((prev) => {
    ...
    await runWithConcurrency(filesToConvert, MAX_PARALLEL_CONVERSIONS, async (file) => {
      ...
      try {
        const queueResult = await queuePdfToJpg(file.file, {
        ...
        const outputName = resolvePdfToJpgOutputName(downloadInfo.fileName, file.name);
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Conversion completed with warnings.';
      setRequestError(message);
    }
  };
              ...
              <div className="sticker-card p-6 space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-3 bg-gray-200 rounded-full appearance-none cursor-pointer accent-violet disabled:opacity-60"
                  />
                </div>
