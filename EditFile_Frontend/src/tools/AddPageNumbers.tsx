...
import { AlertCircle, Download, FileText, Hash, Loader2, Upload, X } from 'lucide-react';
import {
  addPdfPageNumbersFile,
...
export default function AddPageNumbers() {
  ...
  const handleProcess = async () => {
    ...
    if (!Number.isInteger(startNumber) || startNumber < 1) {
      setError('Start number must be 1 or greater.');
      return;
    }
    ...
    try {
      const processed = await addPdfPageNumbersFile(selectedFile, {
        position,
        startNumber,
      });
      setResult(processed);
      ...
      setResult(null);
      setError(requestError instanceof Error ? requestError.message : 'Add page numbers failed');
    } finally {
          ...
          >
            ...
            >
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-violet/10 rounded-2xl flex items-center justify-center mb-6">
                <Hash className="w-8 h-8 sm:w-10 sm:h-10 text-violet" />
              </div>
              <h3 className="font-display font-bold text-xl sm:text-2xl text-dark text-center mb-2">
                Drop PDF file to add page numbers
              </h3>
              <p className="text-gray text-center mb-6">
                Choose number position and generate numbered PDF
              </p>
        ...
        {selectedFile && (
          ...
          >
            <div className="sticker-card p-6 space-y-4">
              <div>
                <label className="font-display font-bold text-dark block mb-3">Position</label>
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={option.value}
                      onClick={() => {
                        setPosition(option.value as NumberPosition);
                        setResult(null);
                      }}
              ...
              <div>
                <label className="font-display font-bold text-dark block mb-2">Start Number</label>
                <input
                  type="number"
                  min={1}
                  value={startNumber}
                  onChange={(event) => {
                    ...
                    setStartNumber(Number.isInteger(nextValue) && nextValue > 0 ? nextValue : 1);
                    setResult(null);
                  }}
                  disabled={isProcessing}
