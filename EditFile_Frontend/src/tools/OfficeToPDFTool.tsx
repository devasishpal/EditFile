...
interface OfficeToPDFToolProps {
...
export default function OfficeToPDFTool({
  ...
  const [files, setFiles] = useState<OfficeToPdfFile[]>([]);
  const [isDragging, setIsDragging] = useState(false);
  const [isConverting, setIsConverting] = useState(false);
  const [requestError, setRequestError] = useState<string | null>(null);
  const [requestSuccess, setRequestSuccess] = useState<string | null>(null);
  const filesRef = useRef<OfficeToPdfFile[]>([]);
  ...
  const handleFiles = useCallback(
    (incomingFiles: File[]) => {
      ...
      if (validFiles.length !== incomingFiles.length) {
        setRequestError(invalidTypeMessage);
      } else {
      ...
      const createdFiles: OfficeToPdfFile[] = validFiles.map((file) => ({
        id: createFileId(),
        file,
        name: file.name,
        size: file.size,
    ...
    [invalidTypeMessage, isValidFile]
  );
  ...
  const handleSingleDownload = (file: OfficeToPdfFile) => {
    if (!file.downloadUrl) {
      return;
    }
  ...
  const autoDownloadZip = async (completedFiles: OfficeToPdfFile[]) => {
    const zip = new JSZip();
  ...
  const handleConvert = async () => {
    ...
    const filesToConvert = files.filter((file) => file.status === 'ready' || file.status === 'error');
    ...
    const updateFileState = (fileId: string, updater: (target: OfficeToPdfFile) => OfficeToPdfFile) => {
      setFiles((prev) => {
    ...
    await runWithConcurrency(filesToConvert, MAX_PARALLEL_CONVERSIONS, async (file) => {
      ...
      try {
        const queueResult = await queueConversion(file.file, (progress) => {
          updateFileState(file.id, (target) => ({
            ...target,
            status: 'uploading',
            progress,
    ...
    const completedFiles = latestFiles.filter((file) => file.status === 'completed' && file.downloadUrl);
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'ZIP download failed after conversion.';
      setRequestError(message);
    }
  };

  const completedFiles = files.filter((file) => file.status === 'completed' && file.downloadUrl);
  const hasPendingConversion = files.some((file) => file.status === 'ready' || file.status === 'error');
  const shouldShowDownloadAction = !isConverting && !hasPendingConversion && completedFiles.length > 0;
            ...
            >
              {(requestError || requestSuccess) && (
                <div className="sticker-card p-4">
                  ...
                  {!requestError && requestSuccess && <p className="text-green-600 text-sm">{requestSuccess}</p>}
                </div>
              )}
                ...
                >
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {file.status === 'ready' && <p className="text-violet text-xs mt-1">Ready to convert</p>}
                      {file.status === 'error' && file.error && <p className="text-red-500 text-xs mt-1">{file.error}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      {file.status === 'uploading' && (
                        <div className="w-24">
                          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div className="h-full bg-violet transition-all duration-300" style={{ width: `${file.progress}%` }} />
                          </div>
                        </div>
                      )}
                      ...
                      {(file.status === 'ready' || file.status === 'error') && <span className="text-sm text-gray">Waiting</span>}
