...
import { AlertCircle, Download, FileText, Loader2, RotateCw, Upload, X } from 'lucide-react';
import {
...
export default function RotatePDF() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const processed = await rotatePdfFile(selectedFile, {
        rotation,
        pages,
      });
      setResult(processed);
      ...
      setResult(null);
      setError(requestError instanceof Error ? requestError.message : 'Rotate failed');
    } finally {
          ...
          >
            ...
            >
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-violet/10 rounded-2xl flex items-center justify-center mb-6">
                <RotateCw className="w-8 h-8 sm:w-10 sm:h-10 text-violet" />
              </div>
              <h3 className="font-display font-bold text-xl sm:text-2xl text-dark text-center mb-2">
                Drop PDF file to rotate
              </h3>
              <p className="text-gray text-center mb-6">Rotate all pages or only selected pages</p>
        ...
        {selectedFile && (
          ...
          >
            <div className="sticker-card p-6 space-y-4">
              <div>
                <label className="font-display font-bold text-dark block mb-3">Rotation</label>
                <div className="flex flex-wrap gap-2">
                  {[90, 180, 270].map((angle) => (
                    <button
                      key={angle}
                      onClick={() => {
                        setRotation(angle as 90 | 180 | 270);
                        setResult(null);
                      }}
              ...
              <div>
                ...
                <input
                  ...
                  disabled={isProcessing}
                  placeholder="All pages by default, or use: 1,3,5-7"
